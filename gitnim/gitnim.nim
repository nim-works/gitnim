...
proc git(args: string; options = defaultProcess): string
  ...
  run("git", args.split, options = options).output
...
proc refresh() =
  ...
  withinDirectory getAppDir().parentDir:
    ...
    if not dirExists dist:
      createDir dist
      git("submodule add --depth 1 $1 dist" % [
        distribution().quoteShell ])
      echo "added submodule: ", distribution()
  withinDistribution:
    echo "fetch and pull distribution"
    git("fetch --all --prune")
    git("pull")
...
when isMainModule:
  ...
  if paramCount() == 0:
    ...
    info git("branch --all --sort=version:refname --verbose --color", {poStdErrToStdOut})
    info "or you can specify one of these tags; eg. `git nim latest`:"
    info git("tag --list -n2 --sort=version:refname --color", {poStdErrToStdOut})
  else:
